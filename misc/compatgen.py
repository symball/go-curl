...
tags = list(filter(lambda t: int("%02d%02d%02d" % tuple(map(int, t.split('-')[1].split('_')))) > 71000, tags))
versions = sorted(tags, key=lambda o:tuple(map(int, o.split('-')[1].split('_'))), reverse=True)
last = version_symbol(versions[0])
...
if __name__ == '__main__':
    for ver in versions:
        major, minor, patch = map(int, ver.split('-')[1].split("_"))
        ...
        result.append(
            "#if (LIBCURL_VERSION_MAJOR == {} && LIBCURL_VERSION_MINOR == {} && LIBCURL_VERSION_PATCH < {}) || (LIBCURL_VERSION_MAJOR == {} && LIBCURL_VERSION_MINOR < {}) || LIBCURL_VERSION_MAJOR < {}".format(
